...
/**
 * Logo URL field plus optional direct file upload for credit cards.
 */
const CardLogoFields = ({
  logoUrl,
  cardId,
  cardName,
  onLogoUrlChange,
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      setUploading(true);
      const updated = await creditCardService.uploadLogo(cardId, file);
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          ...
          {!cardId && ' Logo file will upload when you save a new card.'}
        </p>
      ...
      {previewSrc && (
        <div className="flex items-center gap-3 p-3 rounded-lg border border-border bg-muted/30">
          <img
            src={previewSrc}
            alt={cardName ? `${cardName} logo` : 'Card logo preview'}
