...
import { adminEmployeeLearningService } from '../../../services/employeeLearningService';
...
const CATEGORY_OPTIONS = [
  ...
  { value: 'System Training', label: 'System Training' },
...
const EmployeeLearningTab = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await adminEmployeeLearningService.listAll();
  ...
  const handlePublish = async (e) => {
    ...
    try {
      ...
      fd.append('categoryLabel', form.categoryLabel);
      ...
      await adminEmployeeLearningService.publish(fd);
      setMessage('Published — visible on employee Training tab.');
      setForm(emptyForm);
      setFile(null);
  ...
  const toggleActive = async (item) => {
    await adminEmployeeLearningService.update(item.id, { isActive: !item.isActive });
    await load();
  };
    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-foreground flex items-center gap-2">
          <Icon name="GraduationCap" size={22} className="text-employee-primary" />
          Employee learning hub
        </h2>
        <p className="text-sm text-muted-foreground mt-1 max-w-3xl">
          Publish training videos, PDFs, presentations, and circulars for employees. Content appears
          under Training on the Employee Portal.
        </p>
      </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          />
          <Select
            label="Category (card icon)"
            options={CATEGORY_OPTIONS}
            value={form.categoryLabel}
            onChange={(v) => setForm((p) => ({ ...p, categoryLabel: v }))}
          />
          <Input
            label="Title"
            value={form.title}
        ...
        <label className="flex items-center gap-2 text-sm">
          ...
          />
          Show &quot;NEW&quot; badge on employee portal
        </label>
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        {loading ? (
          ...
          <p className="p-6 text-sm text-muted-foreground">No employee learning content yet.</p>
        ) : (
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-t border-border">
                    ...
                    <td className="px-4 py-3">{item.category || item.categoryLabel}</td>
...
export default EmployeeLearningTab;
